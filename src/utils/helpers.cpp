...
Solution format_solution(const Input& input, const RawSolution& raw_routes) {
  ...
  for (std::size_t i = 0; i < raw_routes.size(); ++i) {
    ...
    service += first_job.service_for_vehicle(v.service_type);
    ...
    ETA += (first_job_setup + first_job.service_for_vehicle(v.service_type));
    ...
    for (std::size_t r = 0; r < route.size() - 1; ++r) {
      ...
      service += current_job.service_for_vehicle(v.service_type);
      ...
      ETA += (current_setup + current_job.service_for_vehicle(v.service_type));
...
Route format_route(const Input& input,
  ...
  for (std::size_t r = tw_r.route.size(); r > 0; --r) {
    ...
    Duration diff = current_setup +
                    previous_job.service_for_vehicle(v.service_type) +
                    remaining_travel_time;
  ...
  for (std::size_t r = 0; r < tw_r.route.size(); ++r) {
    ...
    step_start +=
      (current_setup + current_job.service_for_vehicle(v.service_type));
